...
int Unfold2(int method,int algo,bool useSpectraFromFile=1, bool useMatrixFromFile=1, int doToy = 1, int isMC = 0,char *spectraFileName = "pbpb_spectra_akPu3PF.root",double recoJetPtCut = 60,double trackMaxPtCut = 0) // algo 2 =akpu2 ; 3 =akpu3 ; 4 =akpu4 ;1 = icpu5
{
#ifdef __CINT__
...
//	   boundaries_pthat[9]=1000;
	...
	TCut dataSelection;
        ...
        TCut TriggerSelectionPbPb65;
        TCut TriggerSelectionPbPb55;
        TCut TriggerSelectionPbPbMB;

	if (isMC) {
	         // MC closure test, no reweighting
		fVz = new TF1("fVz","1");
		fCentralityWeight =  new TF1("fCentralityWeight","1");
		dataSelection = Form("abs(vz)<15&&trackMax/jtpt>0.01&&abs(jteta)<2&&jtpt>%.0f&&trackMax>%f",recoJetPtCut,trackMaxPtCut);
	} else {
		...
		dataSelection = Form("abs(vz)<15&&pcollisionEventSelection&&trackMax/jtpt>0.01&&pHBHENoiseFilter&&abs(jteta)<2&&jtpt>%.0f&&trackMax>%f",recoJetPtCut,trackMaxPtCut);
                TriggerSelectionPP = "";//"HLT_Jet60_v1";
                TriggerSelectionPbPb80 ="HLT_HIJet80_v1";
                TriggerSelectionPbPb65 ="HLT_HIJet65_v1";
                TriggerSelectionPbPb55 ="HLT_HIJet55_v1";
                TriggerSelectionPbPbMB ="";
	}
	...
	if (isMC) {
	        if (yinglu) {
	   	   infData = new TFile("/hadoop/store/user/belt/hiForest2/Pythia80_HydjetDrum_mix01_HiForest2_v20.root");
	           ...
	           infData = new TFile("/d102/yjlee/hiForest2MC/Pythia30_HydjetDrum_mix01_HiForest2_v19.root");   
	           infDataMB = new TFile("/d102/yjlee/hiForest2MC/Pythia30_HydjetDrum_mix01_HiForest2_v19.root");   
		}
		cout << "This is a MC closure test"<<endl;
  		   ...
  		   infData = new TFile("sample/HLT_HIJet80_jtpt40_new.root");
  		   infDataMB = new TFile("sample/MB_jtpt40.root");
  		   infDataJet65 = new TFile("sample/HLT_HIJet65_jtpt40.root");
  		}
	...
	if (useSkim) 
	{
	   tDataJet = (TTree*)infData->Get(Form("%sTree",algoName[algo]));
	} else {
	...
	TTree *tDataJet65Evt = (TTree*)infDataJet65->Get("hiEvtAnalyzer/HiTree");
	TTree *tDataJet65Skim = (TTree*)infDataJet65->Get("skimanalysis/HltTree");
	TTree *tDataJet65Hlt = (TTree*)infDataJet65->Get("hltanalysis/HltTree");
	TTree *tDataJet65Jet;
	if (useSkim) 
	{
	   tDataJet65Jet = (TTree*)infDataJet65->Get(Form("%sTree",algoName[algo]));
	} else {
	   tDataJet65Jet = (TTree*)infDataJet65->Get(Form("%sJetAnalyzer/t",algoName[algo]));
	}

	tDataJet65Jet->AddFriend(tDataJet65Evt);
	tDataJet65Jet->AddFriend(tDataJet65Skim);
	tDataJet65Jet->AddFriend(tDataJet65Hlt);


	TTree *tDataMBEvt = (TTree*)infDataMB->Get("hiEvtAnalyzer/HiTree");
	TTree *tDataMBSkim = (TTree*)infDataMB->Get("skimanalysis/HltTree");
	TTree *tDataMBHlt = (TTree*)infDataMB->Get("hltanalysis/HltTree");
	TTree *tDataMBJet;
	if (useSkim) 
	{
	   tDataMBJet = (TTree*)infDataMB->Get(Form("%sTree",algoName[algo]));
	} else {
	   tDataMBJet = (TTree*)infDataMB->Get(Form("%sJetAnalyzer/t",algoName[algo]));
	}

	tDataMBJet->AddFriend(tDataMBEvt);
	tDataMBJet->AddFriend(tDataMBSkim);
	tDataMBJet->AddFriend(tDataMBHlt);
	...
	if (isMC) {
		   ...
		   infPP = new TFile("/d102/yjlee/hiForest2MC/pp276Dijet50_merged.root");
		}
	} else {
	...
	else data_pq = new JetData("/d102/yjlee/hiForest2MC/pp276Dijet30_merged.root", Form("%sJetAnalyzer/t",algoName[algo]),Form("%sJetAnalyzer/t",algoNameGen[algo])); //used pythia for now..
...
/*	
	...
	tRandom->Branch("nref",&dataNJet,"nref/I");
	tRandom->Branch("rndGaus",rndGaus,"rndGaus[nref]/F");
...
//		tDataJet->AddFriend(tRandom);
		tDataJet->Project("hCentData","hiBin",dataSelection);
		tDataJet->Project("hVzData","vz",dataSelection);
	}
...
//			uhist[i]->hMeasMB = (TH1F*)fSpectra->Get(Form("hMeasMB_cent%d",i));
//			if (i!=nbins_cent) mergeSpectra(uhist[i]->hMeasMB,uhist[i]->hMeas,100);
			...
			if (!isMC) {
				...
				tDataMBJet->Project(Form("hMeasMB_cent%d",i),"jtpt", dataSelection&&centCut&&TriggerSelectionPbPbMB);
				tDataJet65Jet->Project(Form("hMeasJet65_cent%d",i),"jtpt", dataSelection&&centCut&&TriggerSelectionPbPb65);
				uhist[i]->hMeasJet80=(TH1F*) uhist[i]->hMeas->Clone(Form("hMeasJet80_cent%d",i));
                                if (algo==2) {
				   mergeSpectra(uhist[i]->hMeasJet65,uhist[i]->hMeas,95,lumi/lumi65);
				   mergeSpectra(uhist[i]->hMeasMB,uhist[i]->hMeas,80,lumi/lumiMB);
				} else if (algo == 3) {
      				   mergeSpectra(uhist[i]->hMeasJet65,uhist[i]->hMeas,95,lumi/lumi65);
				   mergeSpectra(uhist[i]->hMeasMB,uhist[i]->hMeas,80,lumi/lumiMB);
				} else if (algo == 4) {
      				   mergeSpectra(uhist[i]->hMeasJet65,uhist[i]->hMeas,110,lumi/lumi65);
				   mergeSpectra(uhist[i]->hMeasMB,uhist[i]->hMeas,90,lumi/lumiMB);
				}
				tDataJet->Project(Form("hMeasJECSys_cent%d",i),Form("jtpt*%f",1.+0.02/nbins_cent*(nbins_cent-i)), dataSelection&&centCut&&TriggerSelectionPbPb80);
				tDataJet->Project(Form("hMeasSmearSys_cent%d",i),"jtpt+rndGaus", dataSelection&&centCut&&TriggerSelectionPbPb80);
...
//			uhist[i]->hMatrix = (TH2F*) fSpectra->Get(Form("hMatrixFit_cent%d",i));
	...
	if (!isMC) 
		tPPJet->Project(Form("hMeas_cent%d",nbins_cent),"jtpt",dataSelection&&TriggerSelectionPP);
	tPPJet->Project("hVzPPData","vz",dataSelection&&TriggerSelectionPP);
		...
		for (int i=0;i<nbins_pthat;i++) {
			...
			for (Long64_t jentry2=0; jentry2<data[i]->tJet->GetEntries();jentry2++) {
				...
				for (int k= 0; k < data[i]->njets; k++) { 
					...
					if ( data[i]->jteta[k]  > 2. || data[i]->jteta[k] < -2. ) continue;
					if ( data[i]->trackMax[k]/data[i]->jtpt[k]<0.01|| data[i]->jtpt[k] < recoJetPtCut || data[i]->trackMax[k]< trackMaxPtCut ) continue; //
					...
					if ( data[i]->refpt[k]<0) data[i]->refpt[k]=0;
					if (subEvt!=0&&subEvt!=-1) continue;
		...
		for (int i=0;i<nbinsPP_pthat;i++) {
			...
			for (Long64_t jentry2=0; jentry2<dataPP[i]->tJet->GetEntries();jentry2++) {
				...
				for (int k= 0; k < dataPP[i]->njets; k++) { 
					int subEvt=-1;
					if ( dataPP[i]->refpt[k]<0) dataPP[i]->refpt[k]=0;
					if ( dataPP[i]->refpt[k]  < 30. ) continue;
					if ( dataPP[i]->jteta[k]  > 2. || dataPP[i]->jteta[k] < -2. ) continue;
					if ( dataPP[i]->trackMax[k]/dataPP[i]->jtpt[k]<0.01|| dataPP[i]->jtpt[k] < recoJetPtCut || dataPP[i]->trackMax[k]< trackMaxPtCut) continue; 
...
//		if (!useMatrixFromFile) uhist[i]->hMatrixFit = fitMatrix(uhist[i]->hResponseNorm,recoJetPtCut);
	...
	for (int i=0;i<=nbins_cent;i++) {
		...
		prior myPrior(uhist[i]->hMatrixFit,uhist[i]->hMeas,00.5);
		myPrior.unfold(uhist[i]->hMeas,2);
    	        TF1 *fPow = new TF1("fPow","[0]*x^[1]* (1-x/2760*2)^[2]*(x>70)");
                fPow->SetParameters(1e13,-4.5,17);
		uhist[i]->hMeas->Fit("fPow","","",70,300);
		uhist[i]->hMeas->Fit("fPow","","",70,300);
		uhist[i]->hMeas->Fit("fPow","","",70,300);

		bayesianUnfold myUnfoldingPrior(uhist[i]->hMatrixFit,myPrior.hPrior,0);
		myUnfoldingPrior.unfold(uhist[i]->hMeas,nBayesianIter);
		removeZero(myPrior.hPrior);
                myPrior.hPrior->Smooth(5);
...
//		hPrior = (TH1F*)uhist[i]->hMeas->Clone(Form("hPrior_cent%d",i));
		removeZero(hPrior);
		//myPrior.hPrior;//
		//hPrior = (TH1F*)uhist[i]->hMatrixFit->ProjectionX()->Clone(Form("hPrior_cent%d",i));
