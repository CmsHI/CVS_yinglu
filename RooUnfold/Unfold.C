...
void Unfold(int method,int algo,bool useSpectraFromFile=1, bool useMatrixFromFile=1, int doToy = 1, int isMC = 0,char *spectraFileName = "pbpbSpectra.root", int doJECSys = 1) // algo 2 =akpu2 ; 3 =akpu3 ; 4 =akpu4 ;1 = icpu5
{
#ifdef __CINT__
	...
	// Output file
	TFile *pbpb_Unfo = new TFile(Form("pbpb_Unfo_%s.root",algoName[algo]),"RECREATE");
	...
	if (isMC) {
		...
		dataSelection = "abs(vz)<15&&pcollisionEventSelection&&trackMax/jtpt>0.01&&pHBHENoiseFilter&&abs(jteta)<2";
	...
	for (int i=0;i<=nbins_cent;i++){
		...
		if (useSpectraFromFile) {
			...
			if (!isMC) {
				tDataJet->Project(Form("hMeas_cent%d",i),"jtpt", dataSelection&&centCut&&TriggerSelectionPbPb);
				tDataJet->Project(Form("hMeasJECSys_cent%d",i),Form("jtpt*%f",1.+0.02/nbins_cent*(nbins_cent-i)), dataSelection&&centCut&&TriggerSelectionPbPb);
				tDataJet->Project(Form("hMeasSmearSys_cent%d",i),"jtpt+rndGaus", dataSelection&&centCut&&TriggerSelectionPbPb);
	...
	if (!isMC) {
		...
		if(!isMC)
		{
			...
			{	TLegend *leg = myLegend(0.48,0.62,0.92,0.95);
				...
				leg->AddEntry(tTAAerr[0],"T_{AA} + Lumi","f");
